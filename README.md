...
- PostgreSQL + SQLAlchemy 2.x（异步 ORM）
- Redis + RQ（翻译任务队列）
...
| `POST` | `/api/translation-tasks` | 创建翻译任务并入队（异步） |
| `GET` | `/api/translation-tasks/{task_id}` | 查询任务状态 |
| `GET` | `/api/translation-tasks/{task_id}/file-previews` | 查询成功任务的文件映射预览 |
